    ...
    public static void main(String[] args) {
        ...
        for (Map.Entry<String, LocalDate> each : map.entrySet()) {
            if(each.getValue().isBefore(LocalDate.of(2015,1,1))){
                System.out.println("before 2015 "+each.getKey());
            }

            if(each.getValue().isEqual(LocalDate.of(2020,05,15))){
                System.out.println("05/15/2020 "+each.getKey());
            }



            if(each.getValue().isAfter(LocalDate.of(2014,1,1))){
                count++;
            }
        }
